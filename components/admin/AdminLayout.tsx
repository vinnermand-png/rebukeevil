...
export default function AdminLayout({ initialState = 'loading' }: { initialState?: AdminState }) {
  ...
  return <main className="min-h-screen p-6 md:p-12"><header className="mb-16 flex items-center justify-between"><h1 className="display text-3xl">REBUKEEVIL ADMIN</h1><button onClick={() => client?.auth.signOut()} className="mono text-[9px] text-[#85847d]">SIGN OUT</button></header><LinksManager/></main>;
}
