...
export default async function CommunityProfilePage({ params }: { params: Promise<{ username: string }> }) { const access = await getCommunityAccess(); if (!access.allowed) notFound(); const { username } = await params; return <main className="mx-auto min-h-screen max-w-[1540px] px-5 pb-20 md:px-8"><GarrisonTopBar/><div className="mx-auto max-w-[900px] pt-10 md:pt-16"><CommunityProfile username={username}/></div></main>; }
