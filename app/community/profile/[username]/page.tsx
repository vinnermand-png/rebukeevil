...
export default async function CommunityProfilePage({ params }: { params: Promise<{ username: string }> }) { const access = await getCommunityAccess(); if (!access.allowed) notFound(); const { username } = await params; return <main className="mx-auto min-h-screen max-w-[1540px] bg-[#171714] px-5 pb-20 text-[#f0e8d8] md:px-8"><GarrisonTopBar/><div className="mx-auto max-w-[900px] pt-10 md:pt-16"><CommunityProfile username={username}/></div></main>; }
